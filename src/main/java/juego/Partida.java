...
public class Partida implements Serializable {
    ...
    private void inicializarUnidades() {
        ...
        logger.registrarAccion(ing1, "Se posiciona en " + posIng1.toString(), this.turnoGlobal); // Usar this.turnoGlobal
        ...
        logger.registrarAccion(poet1, "Se posiciona en " + posPoet1.toString(), this.turnoGlobal); // Usar this.turnoGlobal
        ...
        logger.registrarAccion(ing2, "Se posiciona en " + posIng2.toString(), this.turnoGlobal); // Usar this.turnoGlobal
        ...
        logger.registrarAccion(poet2, "Se posiciona en " + posPoet2.toString(), this.turnoGlobal); // Usar this.turnoGlobal
    }
    ...
    public boolean moverUnidad(Unidad unidad, Posicion nuevaPosicion) {
        ...
        if (!gestorTurnos.obtenerJugadorActual().getUnidades().contiene(unidad)) {
            logger.registrarAccion(unidad, "Intento de movimiento no autorizado por jugador", this.turnoGlobal); // Usar this.turnoGlobal
            return false;
        }
        ...
        if (casillaDestino == null || casillaDestino.estaOcupada()) {
            logger.registrarAccion(unidad, "Intento de mover a casilla inválida o ocupada: " + nuevaPosicion.toString(), this.turnoGlobal); // Usar this.turnoGlobal
            return false;
        }
        ...
        if (casillasAlcanzables.containsKey(casillaDestino)) {
            ...
            logger.registrarAccion(unidad, "se mueve a " + nuevaPosicion.toString(), this.turnoGlobal); // Usar this.turnoGlobal
            return true;
        } else {
            logger.registrarAccion(unidad, "Intento de movimiento fuera de rango a: " + nuevaPosicion.toString(), this.turnoGlobal); // Usar this.turnoGlobal
            return false;
        }
    }
    ...
    public int atacarUnidad(Unidad atacante, Unidad defensor) {
        ...
        if (!gestorTurnos.obtenerJugadorActual().getUnidades().contiene(atacante)) {
            logger.registrarAccion(atacante, "Intento de ataque no autorizado por jugador", this.turnoGlobal); // Usar this.turnoGlobal
            return 0;
        }
        ...
        if (!jugadorOponente.getUnidades().contiene(defensor)) {
            logger.registrarAccion(atacante, "Intento de ataque a unidad no enemiga o inexistente: " + defensor.getNombre(), this.turnoGlobal); // Usar this.turnoGlobal
            return 0;
        }
        ...
        if (atacante.getPosicion().distanciaA(defensor.getPosicion()) > atacante.getRangoAtaque()) {
            logger.registrarAccion(atacante, "Intento de ataque fuera de rango a " + defensor.getNombre(), this.turnoGlobal); // Usar this.turnoGlobal
        ...
        logger.registrarAccion(atacante, "ataca a " + defensor.getNombre() + " causando " + dano + " de daño. HP restante de " + defensor.getNombre() + ": " + defensor.getHp(), this.turnoGlobal); // Usar this.turnoGlobal
        ...
        if (defensor.getHp() <= 0) {
            ...
            if (jugador1.getUnidades().contiene(defensor)) {
                ...
                logger.registrarAccion(atacante, "elimina a " + defensor.getNombre() + " de " + jugador1.getNombre(), this.turnoGlobal); // Usar this.turnoGlobal
            } else if (jugador2.getUnidades().contiene(defensor)) {
                jugador2.eliminarUnidad(defensor);
                logger.registrarAccion(atacante, "elimina a " + defensor.getNombre() + " de " + jugador2.getNombre(), this.turnoGlobal); // Usar this.turnoGlobal
            }
            logger.registrarAccion(defensor, "es eliminado. HP final: " + defensor.getHp(), this.turnoGlobal); // Usar this.turnoGlobal
        }
        return dano;
    }
    ...
    private void generarNuevaUnidadParaJugadorActual() {
        ...
        if (unidadesJugador.estaVacia()) {
            ...
            if (casillaBase != null && !casillaBase.estaOcupada()) {
                ...
                logger.registrarAccion(nuevaUnidad, "generada y posicionada en base " + posBase.toString(), this.turnoGlobal); // Usar this.turnoGlobal
                return;
            }
        } else {
            ...
            for (int i = 0; i < unidadesJugador.tamano(); i++) {
                ...
                for (int j = 0; j < adyacentes.tamano(); j++) {
                    ...
                    if (adyacente != null && !adyacente.estaOcupada()) {
                        ...
                        jugadorActual.agregarUnidad(nuevaUnidad);
                        logger.registrarAccion(nuevaUnidad, "generada y posicionada al lado de " + unidadExistente.getNombre() + " en " + adyacente.getPosicion().toString(), this.turnoGlobal); // Usar this.turnoGlobal
        ...
        logger.registrarAccion(null, "No se pudo generar una nueva unidad para " + jugadorActual.getNombre() + " por falta de espacio.", this.turnoGlobal); // Usar this.turnoGlobal
    }
